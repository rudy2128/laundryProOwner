...
import com.anthony.laundrypro.ui.reg.RegFirstActivity
...
class LoginActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnReg.setOnClickListener {
            val i = Intent(applicationContext,RegFirstActivity::class.java)
            startActivity(i)
            finish()
        }
        ...
        btnLogin.setOnClickListener {
            ...
            if (phone.isEmpty()){
                ...
                val query = mDatabase.orderByChild("phone").equalTo(phone)
    ...
    private fun nextQuery(password: String, phone: String,code:String) {
        mDatabase = FirebaseDatabase.getInstance("https://laundry-87068-default-rtdb.asia-southeast1.firebasedatabase.app").getReference("laundryPro")
        val query = mDatabase.orderByChild("password").equalTo(password)
        query.addListenerForSingleValueEvent(object:ValueEventListener{
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()){
                    ...
                    Toast.makeText(applicationContext,"Maaf pasword salah!!",Toast.LENGTH_SHORT).show()
                }
            }
